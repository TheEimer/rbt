...
def reset_momentum(momentum, mask):
    return momentum if mask is None else momentum * (1.0 - mask)
...
def weight_reinit_zero(param, mask):
  if mask is None:
    ...
    return jnp.where(mask == 1, new_param, param)
...
class BaseRecycler:
  ...
  def is_update_iter(self, step):
    return step > 0 & (step % self.reset_period == 0)
  ...
  def intersected_dead_neurons_with_last_reset(
    ...
    if self.is_logging_step(update_step):
      return self.log_intersected_dead_neurons(intermediates)
    else:
      return None
  ...
  def log_intersected_dead_neurons(self, intermediates):
    ...
    if self.prev_neuron_score is None:
      ...
      for prev_k_score, current_k_score in zip(
          self.prev_neuron_score.items(), neuron_score_dict.items(), strict=False
      ):
  ...
  def log_dead_neurons_count(self, intermediates):
    ...
    return log_dict(neuron_score, "feature")
...
class LayerReset(BaseRecycler):
  ...
  def is_reset(self, update_step):
    lower_bound = update_step >= self.reset_start_step
    upper_bound = update_step < self.reset_end_step
    within_reset_interval = lower_bound & upper_bound
    return self.is_update_iter(update_step) & within_reset_interval
...
class NeuronRecycler(BaseRecycler):
  ...
  def __init__(
    ...
    super().__init__(all_layers_names, **kwargs)
    ...
    for current_layer, next_layer in zip(
        all_layers_names[:-1], all_layers_names[1:], strict=False
    ):
  ...
  def is_reset(self, update_step):
    interval_lower = update_step >= self.reset_start_step
    interval_upper = update_step < self.reset_end_step
    within_reset_interval = interval_lower & interval_upper
    return self.is_update_iter(update_step) & within_reset_interval

  def is_intermediated_required(self, update_step):
    is_logging = self.is_logging_step(update_step)
    is_update_iter = self.is_update_iter(update_step)
    return jnp.logical_or(is_logging, is_update_iter)
